    ...
    <script>
            ...
            $("#delete").click(function() {
                ...
                Swal.fire({
                    ...
                    if (willDelete.isConfirmed) {
                        $.ajax({
                            type: "POST",
                            url: "{!! route('bendaharapendapatan.kegiatanPendapatan.apiDelete') !!}",
                            data: {
                                _token: "{!! csrf_token() !!}",
                                no_api: 0,
                                id_kegiatan: getId()
        ...
        function tbKegiatanPendapatan() {
            $('#tbKegiatanPendapatan').DataTable({
                ...
                ordering: false,
                ajax: {
                    url: '{{ route('bendaharapendapatan.kegiatanPendapatan.apiGetAll') }}',
                    type: 'GET',
                },
                columns: [{
                        data: 'id_kegiatan',
                        ...
                        render: function(data, type, row, meta) {
                            return `<a href="{!! route('bendaharapendapatan.kegiatanPendapatan.viewUpdate') !!}?id_kegiatan=${row.id_kegiatan}">${data}</a>`;
                        }
                    }, {
                        data: 'periode',
                        name: 'periode',
                        ...
                        render: function(data, type, row, meta) {
                            if(row.id_kegiatan_divisi == null) {
                                return `<a class="btn btn-info btn-sm" href="{!! route('bendaharapendapatan.kegiatanPendapatanMonitoring.viewUpdate') !!}?id_kegiatan=${row.id_kegiatan}"><i class="fas fa-plus"></i></a>`;
                            } else {
                                return `<a class="btn btn-primary btn-sm" href="{!! route('bendaharapendapatan.kegiatanPendapatanMonitoring.viewUpdate') !!}?id_kegiatan=${row.id_kegiatan}">Detail <i class="fas fa-arrow-right"></i></a>`;
                            }
                        }
                    },
                ]
