...
class SPJKegiatanRutinController extends Controller
{   
    private $request;
    private $mSpj;
    private $mDetailSpj;
        ...
        $apiGetAll = DB::select("
            SELECT
                ...
                (
                    SELECT
                        CASE
                            WHEN SUM(dspj.total) IS NULL THEN 0
                            ELSE SUM(dspj.total) END AS total
                    FROM
                        detail_spj AS dspj
                    WHERE
    ...
    {
        $info = [
            'title' => 'SPJ Kegiatan Rutin',
            'site_active' => 'SPJKegiatanRutin',
        ];
        return view('pages._bendaharaPengeluaran.spj.viewGetAll', compact('info'));
    }
    ...
    public function viewDetail()
    {
        ...
        $info = [
            'title' => 'Detail SPJ Kegiatan Rutin',
            'site_active' => 'SPJKegiatanRutin',
        ];
        ...
        return view('pages._bendaharaPengeluaran.spj.viewDetail', compact('info', 'bku', 'detLaks', 'akun', 'spj'));
    }
}
